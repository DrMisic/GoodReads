  ...
  export default {
    ...
    methods: {
      getAutorDetails() {
        ...
        axios.get(`http://localhost:9090/api/autor/id/${autorId}`)
          .then(response => {
            const autor = response.data;
            this.updateDto.ime = autor.ime;
            this.updateDto.prezime = autor.prezime;
            this.updateDto.naslovnaFotografija = autor.profilna_slika;
            this.updateDto.opis = autor.opis;
            this.updateDto.datumRodjenja = autor.datum_rodjenja;
            this.updateDto.mail = autor.email;
